...
globals [ tickCount totalRep hasStarted]
...
to setup
    ...
    create-sellers numberOfSellers [
      ...
      set salesCount 0
      set color blue
      let x (random 451) - 225
      let y (random 451) - 225
      setxy x y
    ]
  ...
  if tickCount = 0 and hasStarted = 1  [
    ask sellers [
      ifelse salesCount < (wealth / 2) [
        set wealth wealth - ((wealth / 2) - salesCount)
      ] [
        if salesCount > (wealth / 2) [
          set wealth wealth + (salesCount - (wealth / 2))
        ]
      ]
...
to move-buyers
  ask buyers [
    ...
    ; todo can't go over the edges
    ifelse patch-here = goal or xcor = 300 or xcor = -300 or ycor = 300 or ycor = -300 [
      set goal one-of patches
    ] [
      walk-towards-goal
    ]

    ; if in radius of a seller, set true
    if pcolor = green [
      set canBuy 0
    ]
  ]
